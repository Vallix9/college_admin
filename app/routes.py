...
from app.utils import export_to_excel, format_date, calculate_age
...
def dashboard():
    ...
    return render_template('dashboard.html', stats=stats, recent_students=recent_students, recent_grades=recent_grades)
...
def logout():
    logout_user()
    flash_msg('info', 'Вы вышли из системы')
    return redirect(url_for('main.login'))
...
def students():
    ...
    students = query.order_by(Student.last_name).paginate(page=page, per_page=20, error_out=False)
    groups = Group.query.order_by(Group.name).all()
    
    return render_template('students.html', students=students, groups=groups, 
                         current_filters={'group': group_id, 'gender': gender, 'status': status, 'search': search})
...
def add_student():
    ...
    if form.validate_on_submit():
        student = Student(
            ...
            patronymic=form.patronymic.data.strip() if form.patronymic.data else None,
            ...
            enrollment_date=form.enrollment_date.data or datetime.now().date()
        )
...
def delete_student(id):
    ...
    db.session.commit()
    flash_msg('warning', f'Студент {student.full_name} удален!')
    return redirect(url_for('main.students'))
...
def add_group():
    ...
    if form.validate_on_submit():
        try:
            ...
            db.session.commit()
            flash_msg('success', f'✅ Группа "{group.name}" успешно создана!')
...
def delete_group(id):
    ...
    if group.students:
        flash_msg('error', f'Нельзя удалить группу {group.name} - в ней есть студенты!')
        return redirect(url_for('main.groups'))
    
    db.session.delete(group)
    db.session.commit()
    flash_msg('warning', f'Группа {group.name} удалена!')
    return redirect(url_for('main.groups'))
...
def grades():
    ...
    grades = query.order_by(Grade.date.desc()).paginate(page=page, per_page=30, error_out=False)
    groups = Group.query.order_by(Group.name).all()
    students = Student.query.order_by(Student.last_name).all()
    
    return render_template('grades.html', grades=grades, groups=groups, students=students,
                         current_filters={'group': group_id, 'student': student_id})
...
def delete_grade(id):
    ...
    db.session.commit()
    flash_msg('warning', 'Оценка удалена!')
    return redirect(url_for('main.grades'))
...
def add_subject():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash_msg('success', f'Предмет "{subject.name}" добавлен!')
        return redirect(url_for('main.subjects'))
    
    return render_template('subject_form.html', form=form, title='Добавление предмета')
...
def report_students():
    ...
    for student in students:
        data.append({
            ...
            'Возраст': calculate_age(student.birth_date) or '',
            ...
            'Дата поступления': format_date(student.enrollment_date)
        })
